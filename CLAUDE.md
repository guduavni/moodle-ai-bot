...
5. **Browser-visible mock Moodle (Coursebot)** — `tests/mock-moodle-coursebot/{index.html,server.js}`. Parallel harness for the `local_coursebot` plugin. The page mimics a real `/course/view.php` (sections, activities) and loads `coursebot/amd/src/chat.js` via the same AMD `define` shim used for QBot. The Node server stands in for `coursebot/ajax.php`: accepts `POST { q }`, returns `{ answer, sessionId? }`. Scenario picker: `success` / `slow-3s` / `401` / `refusal` / `network-error` / `live` (skytutor direct) / `live-proxy` (through deployed `api/chat`). Default port `3031`. Run via `npm run mock:moodle:coursebot`.
